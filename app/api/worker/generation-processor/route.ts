...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('[DEBUG WORKER] ========== Token Validation ==========');
    console.error('[DEBUG WORKER] Auth header exists:', !!authHeader);
    console.error('[DEBUG WORKER] Auth header value:', authHeader);
    console.error('[DEBUG WORKER] Auth token extracted:', authToken);
    console.error('[DEBUG WORKER] Auth token length:', authToken?.length);
    console.error('[DEBUG WORKER] Auth token prefix:', authToken?.substring(0, 10));
    console.error('[DEBUG WORKER] Expected token exists:', !!expectedToken);
    console.error('[DEBUG WORKER] Expected token length:', expectedToken?.length);
    console.error('[DEBUG WORKER] Expected token prefix:', expectedToken?.substring(0, 10));
    console.error('[DEBUG WORKER] Tokens match (exact):', authToken?.trim() === expectedToken?.trim());
    console.error('[DEBUG WORKER] ================================================');
    ...
    console.error('[DEBUG WORKER] ✅ Token validation successful');
