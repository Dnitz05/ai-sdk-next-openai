...
// --- Component per Renderitzar la Resposta de Google Document AI (Versió Anterior Millorada) ---
function GoogleDocumentRenderer({ document }: { document: any }) {
  if (!document || !document.pages || !document.text) { return <p className="text-center text-gray-500 italic py-10">No hi ha dades del document per mostrar o l'estructura rebuda és invàlida.</p>; }
  const fullText = document.text;
  const getText = (textAnchor: any): string => { if (!textAnchor?.textSegments || !fullText) { return ''; } let extractedText = ''; for (const segment of textAnchor.textSegments) { const startIndex = parseInt(segment?.startIndex || '0', 10); const endIndex = parseInt(segment?.endIndex || '0', 10); if (!isNaN(startIndex) && !isNaN(endIndex) && startIndex >= 0 && endIndex >= startIndex && endIndex <= fullText.length) { extractedText += fullText.substring(startIndex, endIndex); } else { console.warn("Segment d'índex invàlid:", segment, `Text Length: ${fullText.length}`); } } return extractedText; };
  const renderTable = (table: any, pageIndex: number, tableIndex: number) => { return ( <div key={`page-${pageIndex}-table-${tableIndex}`} className="overflow-x-auto my-6 shadow-lg border border-gray-300 rounded-lg bg-white"> <table className="min-w-full text-sm border-collapse border border-slate-400"> {table.headerRows?.map((hRow: any, hrIndex: number) => ( <thead key={`thead-${hrIndex}`} className="bg-slate-100"> <tr className="border-b-2 border-slate-300"> {hRow.cells?.map((cell: any, cIndex: number) => ( <th key={`th-${hrIndex}-${cIndex}`} className="border border-slate-300 px-4 py-2 text-left font-semibold text-slate-700 whitespace-nowrap" colSpan={cell.layout?.colSpan && cell.layout.colSpan > 1 ? cell.layout.colSpan : undefined} rowSpan={cell.layout?.rowSpan && cell.layout.rowSpan > 1 ? cell.layout.rowSpan : undefined} > {getText(cell.layout?.textAnchor)} </th> ))} </tr> </thead> ))} <tbody className="divide-y divide-slate-200"> {table.bodyRows?.map((bRow: any, brIndex: number) => ( <tr key={`brow-${brIndex}`} className="hover:bg-slate-50"> {bRow.cells?.map((cell: any, cIndex: number) => ( <td key={`bcell-${brIndex}-${cIndex}`} className="border border-slate-300 px-4 py-2 align-top" colSpan={cell.layout?.colSpan && cell.layout.colSpan > 1 ? cell.layout.colSpan : undefined} rowSpan={cell.layout?.rowSpan && cell.layout.rowSpan > 1 ? cell.layout.rowSpan : undefined} > <span className="whitespace-pre-wrap">{getText(cell.layout?.textAnchor)}</span> </td> ))} </tr> ))} </tbody> </table> </div> ); };
  return ( <div className="space-y-10 font-serif text-gray-900 text-sm md:text-base leading-relaxed"> {document.pages.map((page: any, pageIndex: number) => ( <div key={`page-${pageIndex}`} className="page-content mb-8"> {document.pages.length > 1 && ( <p className="text-center text-xs font-semibold text-gray-500 my-6 pb-2 border-b border-dashed border-gray-300">--- PÀGINA {pageIndex + 1} ---</p> )} {page.paragraphs?.map((paragraph: any, pIndex: number) => ( <p key={`p-${pageIndex}-${pIndex}`} className="mb-4 leading-relaxed text-justify"> {getText(paragraph.layout?.textAnchor)} </p> ))} {page.lists?.map((list: any, lIndex: number) => { const ListTag = 'ul'; const listStyle = 'list-disc'; return ( <ListTag key={`list-${pageIndex}-${lIndex}`} className={`${listStyle} list-outside ml-6 md:ml-8 mb-4 space-y-1.5`}> {list.listItems?.map((item: any, iIndex: number) => ( <li key={`item-${pageIndex}-${lIndex}-${iIndex}`}> {getText(item.layout?.textAnchor)} </li> ))} </ListTag> ); })} {page.tables?.map((table: any, tIndex: number) => renderTable(table, pageIndex, tIndex) )} </div> ))}
      {/* Secció JSON DEBUG - S'HA AFEGIT EL BOTÓ COPIAR AQUÍ A SOTA */}
      <div className="mt-12 pt-6 border-t border-gray-300">
          {/* Contenidor per al títol i el botó */}
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-lg font-semibold text-gray-700">Resposta JSON Completa (Debug)</h3>
            {/* El botó de copiar només apareix si hi ha resultats */}
            {document && (
                 <button
                   onClick={async () => { // Funció inline per copiar
                     const jsonString = JSON.stringify(document, null, 2);
                     try {
                       await navigator.clipboard.writeText(jsonString);
                       // Aquí podríem afegir un estat temporal per mostrar "Copiat!"
                       // De moment, fem un log a la consola del navegador
                       console.log("JSON copiat al porta-retalls!");
                       alert("JSON copiat al porta-retalls!"); // Alerta simple per confirmació
                     } catch (err) {
                       console.error("Error al copiar JSON:", err);
                       alert("Error: No s'ha pogut copiar el JSON.");
                     }
                   }}
                   className="text-xs bg-gray-600 hover:bg-gray-700 text-white px-3 py-1 rounded-md transition-colors duration-150"
                 >
                   Copiar JSON
                 </button>
            )}
...
export default function Page() {
  // Estats (igual que abans)
  const [images, setImages] = useState<string[]>([]);
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);
  const [isLoadingUpload, setIsLoadingUpload] = useState(false);
  const [isLoadingAnalysis, setIsLoadingAnalysis] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [analysisResult, setAnalysisResult] = useState<any | null>(null); // Guarda la resposta JSON de Google

  // Funció millorada per gestionar respostes API (igual que abans)
  ...
  // Gestor de pujada PDF (igual que abans)
  ...
  // Gestor d'anàlisi (igual que abans)
  ...
  // Renderització JSX (estructura general igual, però crida al GoogleDocumentRenderer actualitzat)
  return (
     <main className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 bg-gradient-to-b from-gray-100 to-blue-100 min-h-screen">
       ...
       {/* ===== SECCIÓ RESULTATS DE GOOGLE DOCUMENT AI ===== */}
