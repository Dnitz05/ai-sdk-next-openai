...
interface AiInstruction { id: string; prompt: string; originalText?: string; } // ID és de l'span ai-prompt-target


export default function Home() {
    // --- Estats (Tornem a necessitar AI Temp) ---
    ...
    // Estats per IA (tornem a selecció lliure)
    const [aiSelectedText, setAiSelectedText] = useState<string | null>(null); // Text seleccionat temporalment
    const [aiTemporaryMarkerId, setAiTemporaryMarkerId] = useState<string | null>(null); // ID de l'span temporal
    const [aiUserPrompt, setAiUserPrompt] = useState<string>(''); // Prompt de l'usuari
    const [aiInstructions, setAiInstructions] = useState<AiInstruction[]>([]); // Instruccions guardades
    // Ja no necessitem aiTargetParagraphId
    // Estats Guardar
    ...
    // Eliminat useEffect per obrir sidebar
    ...
    // S'activa en deixar anar el ratolí sobre el contingut DOCX -> Gestiona AMBDÓS casos
    const handleTextSelection = () => {
        // No actuar si no hi ha contingut carregat
        ...
        // Cas 2: Preparar per Instrucció IA (selectedExcelHeader és null)
            ...
            // Neteja marcador temporal IA anterior (si n'hi ha)
            const changedHtml = unwrapTemporaryAiSpan(aiTemporaryMarkerId);
            // Llegim l'HTML actualitzat SI s'ha modificat
            let currentHtml = changedHtml && contentRef.current ? contentRef.current.innerHTML : convertedHtml;

            // Generem el nou ID temporal i l'span
            const tempId = `temp-ai-${Date.now()}`;
            const tempSpan = document.createElement('span');
            tempSpan.dataset.aiTempId = tempId;

            try {
                // Important: Hem de tornar a obtenir el range sobre l'HTML possiblement modificat
                // per evitar errors si unwrapTemporaryAiSpan ha alterat l'estructura.
                // Per simplificar, confiem que la selecció encara és vàlida, però si dóna problemes,
                // caldria re-calcular el range o indicar a l'usuari que torni a seleccionar.
                const newRange = selection.getRangeAt(0); // Intentem reutilitzar range actual
                ...
                // Si falla, restaurem l'HTML anterior si el tenim
                if(changedHtml && baseHtml) setConvertedHtml(baseHtml);
                setAiSelectedText(null); setAiTemporaryMarkerId(null);
            } finally { selection.removeAllRanges(); }
        }
    };

    // Eliminada handleContentClick, ja no es basa en clic a paràgraf

    // Funció per tancar el sidebar (si implementem botó) o reset
    const handleResetSidebar = () => { // Abans handleCloseSidebar
         // Neteja marcador temporal IA si n'hi ha en tancar/resetejar
    ...
    const handleSaveAiInstruction = () => {
        if (!aiUserPrompt.trim() || !aiTemporaryMarkerId || !contentRef.current) { alert("Escriu instrucció."); return; }
        ...
        if (tempSpan) {
            ...
            const originalText = aiSelectedText; // Guardem text original
            // Modifiquem l'span per fer-lo permanent
            tempSpan.removeAttribute('data-ai-temp-id');
            tempSpan.setAttribute('data-ai-instruction-id', instructionId);
            tempSpan.className = 'ai-prompt-target'; // Aplica classe visual permanent
            ...
            console.error("No marcador temporal IA per guardar."); alert("Error guardant instrucció.");
            setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt(''); // Neteja igualment
        }
    };
                            ...
                            <div className="flex-grow overflow-y-auto space-y-4 pr-1 mb-4">
                                ...
                                {convertedHtml && (
                                     <div className="p-3 border rounded border-gray-200">
                                        ...
                                        {!aiSelectedText && ( <p className="text-xs text-gray-400 italic p-2 text-center">...</p> )}

                                        {/* Historial Instruccions IA Guardades */}
                                        {aiInstructions.length > 0 && (
                                            <div className="mt-4 pt-4 border-t">
                                                <h4 className="text-sm font-medium ...">Instruccions Guardades:</h4>
                                                <ul className="space-y-2 text-xs ...">
                                                    {aiInstructions.map((instr, index) => ( <li key={instr.id} className="p-2 border rounded ..."> <span className="block font-medium ...">Instrucció {index + 1}</span> <i className='block ...'>{instr.prompt}</i> <span className='block text-gray-500 ...'>(ID: {instr.id.substring(9,14)}...)</span> </li> ))}
                                                </ul>
                                            </div>
                                        )}
                                    </div>
