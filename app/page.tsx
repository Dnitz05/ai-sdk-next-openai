...
export default function Home() {
  ...
  const triggerUpload = async (file: File) => {
    ...
    setExcelHeaders([]); setSelectedExcelHeader(null); setIsLinkerSidebarOpen(false); setLinks([]); // Reset complet
  ...
  const handleExcelFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setExcelError(null);
      setExcelData(null);
      // Quan es (re)carrega excel dins el sidebar, reseteja vinculació, PERÒ NO TANQUIS EL SIDEBAR
      // setIsLinkerSidebarOpen(false); // <<< LÍNIA ELIMINADA
...
} // <<< Assegura't que aquesta clau tanca el component Home