...
export default function Home() {
  ...
  const [isMounted, setIsMounted] = useState(false); // Estat per renderitzat client (solució error #418)
  ...
  const triggerUpload = async (file: File) => {
    ...
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
          ...
          // console.log("Received data:", data); // Log de depuració general si cal
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      // Comprova per MIME type O extensió per més seguretat
      ...
      if (isValidType) {
        setError(null); triggerUpload(file);
      } else {
        setError('Si us plau, selecciona un fitxer .docx'); setConvertedHtml(null); setMammothMessages([]); setSelectedFileName('Cap fitxer seleccionat');
      }
  ...
  // JSX
  return (
    <main className="flex min-h-screen w-full flex-col items-center p-4 sm:p-8 bg-gray-100"> {/* Fons gris molt clar */}

      {/* Capçalera WEB (oculta en imprimir) */}
      <div className="web-header w-full max-w-2xl mx-auto flex items-center justify-between mb-4 sm:mb-6 px-1"> {/* Amplada max-w-2xl */}
        ...
        <div>
          ...
          <input
            ...
            // --- ATRIBUT ACCEPT SIMPLIFICAT ---
            accept=".docx"
            // ---------------------------------
            className="hidden" // Amaguem l'input per defecte, s'activa amb el label
            disabled={isLoading}
          />
        </div>
      </div>
      ...
      {/* "Foli" Blanc */}
      <div className="print-content w-full max-w-2xl bg-white shadow-lg rounded-sm p-8 md:p-12 lg:p-16 my-4"> {/* Amplada max-w-2xl */}
        ...
        {/* Àrea de Resultats */}
        <div className="mt-1">
          {/* Renderitzat condicional només al client per evitar error #418 */}
          {isMounted && convertedHtml ? (
            ...
            // Missatge inicial o si hi ha error/càrrega
