...
// --- Component Millorat v4: Renderitza HTML d'OpenAI per a Taules ---
// ***** CORRECCIÓ AQUÍ: Afegit 'reformattedTables' a les props *****
function GoogleDocumentRenderer({ document, reformattedTables }: { document: any, reformattedTables: any[] }) {
  ...
  const findReformattedTableHtml = (pageIdx: number, tableIdx: number): string | null => {
    const found = reformattedTables.find(
        // Comprovem que pageIndex i tableIndexOnPage existeixen abans de comparar
        (t) => t && typeof t.pageIndex === 'number' && typeof t.tableIndexOnPage === 'number' &&
               t.pageIndex === pageIdx && t.tableIndexOnPage === tableIdx
    );
    return (found?.html && !found.html.includes("Error processant taula")) ? found.html : null;
  };
    ...
    <div className="space-y-10 font-serif text-gray-900 text-sm md:text-base leading-relaxed">
      {document.pages.map((page: any, pageIndex: number) => (
        <div key={`page-${pageIndex}`} className="page-content mb-8">
          ...
          {page.paragraphs?.map((paragraph: any, pIndex: number) => (
            <p key={`p-${pageIndex}-${pIndex}`} className="mb-4 leading-relaxed text-justify">
               {getText(paragraph.layout?.textAnchor)}
            </p>
          ))}
           {page.lists?.map((list: any, lIndex: number) => {
              const ListTag = 'ul'; const listStyle = 'list-disc';
              return ( <ListTag key={`list-${pageIndex}-${lIndex}`} className={`${listStyle} list-outside ml-6 md:ml-8 mb-4 space-y-1.5`}> {list.listItems?.map((item: any, iIndex: number) => ( <li key={`item-${pageIndex}-${lIndex}-${iIndex}`}> {getText(item.layout?.textAnchor)} </li> ))} </ListTag> );
           })}
           ...
           {page.tables?.map((table: any, tIndex: number) => {
              ...
              if (tableHtml) {
                  return ( <div key={`table-html-${pageIndex}-${tIndex}`} className="my-6 rendered-html-table overflow-x-auto" dangerouslySetInnerHTML={{ __html: tableHtml }} /> );
              } else {
                  // Fallback si no hi ha HTML reformatat per a aquesta taula
                  console.warn(`No s'ha trobat HTML reformatat per a taula ${tIndex} a pàgina ${pageIndex}`);
                  return <div key={`table-fallback-${pageIndex}-${tIndex}`} className="my-6 p-4 border border-red-300 bg-red-50 text-red-700 text-sm rounded"> [ No s'ha pogut renderitzar la taula {tIndex + 1} (Pàg. {pageIndex + 1}) amb el format simplificat d'OpenAI. ] </div>;
              }
           })}
      ...
      <div className="mt-12 pt-6 border-t border-gray-300">
          <div className="flex justify-between items-center mb-3">
            ...
            <button onClick={() => { if (!document) return; try { const jsonString = JSON.stringify(document, null, 2); const blob = new Blob([jsonString], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = window.document.createElement('a'); a.href = url; a.download = 'google-doc-ai-full-response.json'; window.document.body.appendChild(a); a.click(); setTimeout(() => { window.document.body.removeChild(a); URL.revokeObjectURL(url); console.log("Recurs descàrrega JSON netejat."); }, 100); console.log("Descàrrega JSON iniciada."); } catch (error) { console.error("Error creant descàrrega JSON:", error); alert("Error: No s'ha pogut iniciar la descàrrega del JSON."); } }} className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md transition-colors duration-150 disabled:opacity-50" disabled={!document} > Descarregar JSON </button>
          </div>
...
export default function Page() {
  ...
  const [analysisResult, setAnalysisResult] = useState<any | null>(null); // Google Doc Object
  const [reformattedTables, setReformattedTables] = useState<any[]>([]); // Array of { pageIndex, tableIndexOnPage, html }
  ...
  // Renderització JSX (ara passa 'reformattedTables' al renderer)
  return (
     <main className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 bg-gradient-to-b from-gray-100 to-blue-100 min-h-screen">
       ...
       {/* ===== SECCIÓ RESULTATS (ARA PASSA reformattedTables) ===== */}
       {analysisResult && !isLoadingUpload && !isLoadingAnalysis && (
         <section aria-labelledby="results-section-title" className="mt-10 mb-10">
           ...
           <article aria-label="Document Analitzat" className="bg-white p-6 md:p-10 rounded-xl border border-gray-200 shadow-2xl">
              {/* Passem AMBDÓS: el document de Google i les taules HTML */}
