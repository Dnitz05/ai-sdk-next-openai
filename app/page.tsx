...
export default function Home() {
    ...
    const triggerUpload = async (file: File) => {
        ...
        try {
            ...
            console.error("Error processant DOCX:", err); setDocxError(err instanceof Error ? err.message : 'Error desconegut'); setConvertedHtml(null);
                    ...
                    <aside className="w-80 flex-shrink-0 my-4 relative">
                        <div className="sticky top-4 p-4 bg-white rounded shadow-lg border max-h-[calc(100vh-2rem)] overflow-y-auto flex flex-col">
                            ...
                            <div className="flex-grow overflow-y-auto space-y-4 pr-1 mb-4">
                                ...
                                {excelHeaders.length > 0 && (
                                    <div>
                                        <p className="text-sm font-medium text-gray-700 mb-1">Pas 2: Vincular amb Excel</p>
                                        <p className="text-xs text-gray-600 mb-2">1. Clica una capçalera:</p>
                                        <div className="flex flex-col gap-1 mb-4 max-h-48 overflow-y-auto pr-1 border rounded p-2 bg-gray-50">
                                            {excelHeaders.map(header => {
                                                ...
                                                return (
                                                    <button key={header} onClick={() => handleSelectHeader(header)} className={`w-full text-left px-2 py-1 border rounded text-xs font-medium transition-colors break-words flex justify-between items-center ${selectedExcelHeader === header ? 'bg-blue-500 text-white border-blue-600 ring-2 ring-blue-300' : isLinked ? 'bg-green-50 text-green-800 border-green-300 hover:bg-green-100' : 'bg-white text-blue-700 border-blue-300 hover:bg-blue-100'}`} >
                                                        <span>{header}</span>
                                                        {count > 0 && (<span className={`ml-2 text-xs font-normal px-1.5 py-0.5 rounded ${selectedExcelHeader === header ? 'bg-white text-blue-700' : 'bg-gray-200 text-gray-600'}`}> ({count}) </span>)}
                                                    </button>
                                                );
                                            })}
                                        </div>
