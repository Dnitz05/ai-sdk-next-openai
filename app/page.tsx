...
// --- Component GoogleDocumentRenderer (sense canvis respecte l'última versió) ---
// Aquest component ara rebrà correctament l'objecte 'google_document'
function GoogleDocumentRenderer({ document }: { document: any }) {
  // ... (El codi del renderer de la resposta #170 / #176 es manté igual de moment) ...
  if (!document || !document.pages || !document.text) { return <p className="text-center text-gray-500 italic py-10">No hi ha dades del document per mostrar o l'estructura rebuda és invàlida.</p>; } const fullText = document.text; const getText = (textAnchor: any): string => { if (!textAnchor?.textSegments || !fullText) { return ''; } let extractedText = ''; for (const segment of textAnchor.textSegments) { const startIndex = parseInt(segment?.startIndex || '0', 10); const endIndex = parseInt(segment?.endIndex || '0', 10); if (!isNaN(startIndex) && !isNaN(endIndex) && startIndex >= 0 && endIndex >= startIndex && endIndex <= fullText.length) { extractedText += fullText.substring(startIndex, endIndex); } else { console.warn("Segment d'índex invàlid:", segment, `Text Length: ${fullText.length}`); } } return extractedText; }; const renderTable = (table: any, pageIndex: number, tableIndex: number) => { return ( <div key={`page-${pageIndex}-table-${tableIndex}`} className="overflow-x-auto my-6 shadow-lg border border-gray-300 rounded-lg bg-white"> <table className="min-w-full text-sm border-collapse border border-slate-400"> {table.headerRows?.map((hRow: any, hrIndex: number) => ( <thead key={`thead-${hrIndex}`} className="bg-slate-100"> <tr className="border-b-2 border-slate-300"> {hRow.cells?.map((cell: any, cIndex: number) => ( <th key={`th-${hrIndex}-${cIndex}`} className="border border-slate-300 px-4 py-2 text-left font-semibold text-slate-700 whitespace-nowrap" colSpan={cell.layout?.colSpan && cell.layout.colSpan > 1 ? cell.layout.colSpan : undefined} rowSpan={cell.layout?.rowSpan && cell.layout.rowSpan > 1 ? cell.layout.rowSpan : undefined} > {getText(cell.layout?.textAnchor)} </th> ))} </tr> </thead> ))} <tbody className="divide-y divide-slate-200"> {table.bodyRows?.map((bRow: any, brIndex: number) => ( <tr key={`brow-${brIndex}`} className="hover:bg-slate-50"> {bRow.cells?.map((cell: any, cIndex: number) => ( <td key={`bcell-${brIndex}-${cIndex}`} className="border border-slate-300 px-4 py-2 align-top" colSpan={cell.layout?.colSpan && cell.layout.colSpan > 1 ? cell.layout.colSpan : undefined} rowSpan={cell.layout?.rowSpan && cell.layout.rowSpan > 1 ? cell.layout.rowSpan : undefined} > <span className="whitespace-pre-wrap">{getText(cell.layout?.textAnchor)}</span> </td> ))} </tr> ))} </tbody> </table> </div> ); }; return ( <div className="space-y-10 font-serif text-gray-900 text-sm md:text-base leading-relaxed"> {document.pages.map((page: any, pageIndex: number) => ( <div key={`page-${pageIndex}`} className="page-content mb-8"> {document.pages.length > 1 && ( <p className="text-center text-xs font-semibold text-gray-500 my-6 pb-2 border-b border-dashed border-gray-300">--- PÀGINA {pageIndex + 1} ---</p> )} {page.paragraphs?.map((paragraph: any, pIndex: number) => ( <p key={`p-${pageIndex}-${pIndex}`} className="mb-4 leading-relaxed text-justify"> {getText(paragraph.layout?.textAnchor)} </p> ))} {page.lists?.map((list: any, lIndex: number) => { const ListTag = 'ul'; const listStyle = 'list-disc'; return ( <ListTag key={`list-${pageIndex}-${lIndex}`} className={`${listStyle} list-outside ml-6 md:ml-8 mb-4 space-y-1.5`}> {list.listItems?.map((item: any, iIndex: number) => ( <li key={`item-${pageIndex}-${lIndex}-${iIndex}`}> {getText(item.layout?.textAnchor)} </li> ))} </ListTag> ); })} {page.tables?.map((table: any, tIndex: number) => renderTable(table, pageIndex, tIndex) )} </div> ))} {/* Secció JSON DEBUG AMB BOTÓ DESCARREGAR (modificada lleugerament per claredat) */} <div className="mt-12 pt-6 border-t border-gray-300"> <div className="flex justify-between items-center mb-3"> <h3 className="text-lg font-semibold text-gray-700">Resposta JSON Completa de Google (Debug)</h3> <button onClick={() => { if (!document) return; const jsonString = JSON.stringify(document, null, 2); const blob = new Blob([jsonString], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'google-doc-ai-full-response.json'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }} className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md transition-colors duration-150 disabled:opacity-50" disabled={!document} > Descarregar JSON </button> </div> <details className="bg-gray-800 text-white rounded-lg shadow-inner border border-gray-600"> <summary className="text-sm text-gray-400 p-3 cursor-pointer hover:bg-gray-700 rounded-t-lg">Mostra/Amaga JSON</summary> <pre className="whitespace-pre-wrap text-xs text-green-400 p-4 overflow-x-auto font-mono max-h-[600px]"> {JSON.stringify(document, null, 2)} </pre> </details> </div> </div> );
}
...
export default function Page() {
  ...
  // Estat per guardar la resposta COMPLETA de Google Document AI
  const [analysisResult, setAnalysisResult] = useState<any | null>(null);
  // NOU ESTAT per guardar les taules HTML reformatades per OpenAI
  const [reformattedTables, setReformattedTables] = useState<any[]>([]); // Guarda { pageIndex, tableIndexOnPage, html }

  // Funció millorada per gestionar respostes API (igual)
  ...
  // Gestor d'anàlisi (CORREGIT per guardar google_document i reformatted_tables)
  const handleAnalyzeDocument = async () => {
    if (!pdfUrl) { setError("Error intern: No hi ha URL de PDF."); return; }
    setIsLoadingAnalysis(true); setError(null);
    setAnalysisResult(null); // Neteja resultat Google anterior
    setReformattedTables([]); // Neteja taules HTML anteriors
    ...
    try {
      ...
      console.log("Frontend: Dades rebudes:", data); // Log per veure tota la resposta

       // === CORRECCIÓ VALIDACIÓ i GUARDAT ===
       // Validació CORRECTA: comprovem 'google_document'
       if (!data || !data.google_document || typeof data.google_document !== 'object') {
          console.error("Frontend: Resposta invàlida:", data);
          throw new Error("La resposta de l'API d'anàlisi no té l'estructura esperada ('google_document').");
       }
       // Guardem l'objecte 'google_document' a analysisResult
       setAnalysisResult(data.google_document);
       // Guardem l'array de taules HTML (pot ser buit si no n'hi ha o OpenAI falla)
  ...
  return (
    <main className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 bg-gradient-to-b from-gray-100 to-blue-100 min-h-screen">
      ...
      {/* ... Secció Càrrega (igual que abans) ... */}
      ...
      {/* ... Secció Accions (igual que abans) ... */}
      ...
      {/* ===== SECCIÓ RESULTATS (Crida al GoogleDocumentRenderer que rep l'estat correcte) ===== */}
      {analysisResult && !isLoadingUpload && !isLoadingAnalysis && (
        <section aria-labelledby="results-section-title" className="mt-10 mb-10">
           ...
           {/* Mostrem error d'anàlisi si n'hi ha hagut */}
           {error && isLoadingAnalysis === false && <p className="text-center text-red-600 font-semibold mb-6">⚠️ Error Anàlisi: {error}</p>}
           {/* Passem l'objecte 'document' de Google al renderer */}
           <article aria-label="Document Analitzat" className="bg-white p-6 md:p-10 rounded-xl border border-gray-200 shadow-xl">
