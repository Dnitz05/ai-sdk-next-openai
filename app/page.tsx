...
interface Link { id: string; excelHeader: string; selectedText: string; aiPrompt?: string; }
...
export default function Home() {
    ...
    useEffect(() => { /* ... (sense canvis) ... */
        if (prevIsLoadingDocx.current && !isLoadingDocx && isMounted && convertedHtml && !docxError) { setIsLinkerSidebarOpen(true); setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt(''); setSaveStatus('idle'); setSaveMessage(null); } prevIsLoadingDocx.current = isLoadingDocx;
    ...
    const triggerUpload = async (file: File) => { /* ... (sense canvis) ... */
        setIsLoadingDocx(true); setDocxError(null); setConvertedHtml(null); setMammothMessages([]); setSelectedExcelFileName(null); setExcelData(null); setExcelError(null); setExcelHeaders([]); setSelectedExcelHeader(null); setIsLinkerSidebarOpen(false); setLinks([]); setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt(''); setAiInstructions([]); setSaveStatus('idle'); setSaveMessage(null); const formData = new FormData(); formData.append('file', file); try { const r=await fetch('/api/process-document',{method:'POST',body:formData});const ct=r.headers.get("content-type");if(!r.ok){let e:any={error:`E: ${r.status}`};try{e=await r.json();}catch{}throw new Error(e.error||`E ${r.status}`);}if(ct?.includes("application/json")){const d=await r.json();setConvertedHtml(d.html);setMammothMessages(d.messages||[]);}else{throw new Error("Format resposta inesperat.");}}catch(err){console.error("E DOCX:",err);setDocxError(err instanceof Error?err.message:'Error');setConvertedHtml(null);}finally{setIsLoadingDocx(false);}
    };
    const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => { /* ... (sense canvis) ... */
         if (event.target.files && event.target.files[0]) { const f=event.target.files[0]; setSelectedFileName(f.name); const vT=f.type==='application/vnd.openxmlformats-officedocument.wordprocessingml.document'||f.name.toLowerCase().endsWith('.docx'); if(vT){ setDocxError(null); triggerUpload(f); } else { setDocxError('Selecciona .docx'); setConvertedHtml(null); setMammothMessages([]); setSelectedFileName(''); setSelectedExcelFileName(null); setExcelData(null); setExcelError(null); setExcelHeaders([]); setSelectedExcelHeader(null); setIsLinkerSidebarOpen(false); setLinks([]); setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt(''); setAiInstructions([]); setSaveStatus('idle'); setSaveMessage(null); } } else { setSelectedFileName(null); } event.target.value = '';
    };
        ...
        if (tempId && contentRef.current) {
            ...
            if (tempSpan) {
                try {
                    ...
                    tempSpan.replaceWith(...childNodesArray); // Utilitza array
                    // ======================
                    // Retorna true si s'ha fet el canvi (per actualitzar HTML si cal)
                    return true;
                } catch (error) {
                     console.error("Error desembolcallant span temporal:", error);
                }
            }
        }
    ...
    const handleSelectHeader = (header: string) => {
        ...
        if (htmlChanged && contentRef.current) {
            // Actualitzem l'HTML només si s'ha desembolicat alguna cosa
    ...
    const handleTextSelection = () => {
        ...
        if (selectedExcelHeader && selection && !selection.isCollapsed && selection.rangeCount > 0 && contentRef.current) {
             // ... (Lògica vinculació sense canvis) ...
             ...
             if(changed && contentRef.current) setConvertedHtml(contentRef.current.innerHTML); // Actualitza si es neteja
             setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt('');
        }
        // Cas 2: Preparar per Instrucció IA
            ...
            // Neteja marcador temporal IA anterior (ja no cal perquè ho fa handleSelectHeader o handleClose)
            // const changed = unwrapTemporaryAiSpan(aiTemporaryMarkerId);
            // if(changed && contentRef.current) setConvertedHtml(contentRef.current.innerHTML); // Potser innecessari
            ...
            try {
                ...
                setAiUserPrompt('');
                setAiResult(null); // Si tinguéssim resultats, netejar aquí
    ...
    const handleCloseSidebar = () => {
        // Neteja marcador temporal IA si n'hi ha en tancar
         const changed = unwrapTemporaryAiSpan(aiTemporaryMarkerId);
         if(changed && contentRef.current) setConvertedHtml(contentRef.current.innerHTML); // Actualitza HTML
    ...
    const handleSaveAiInstruction = () => {
        if (!aiUserPrompt.trim() || !aiTemporaryMarkerId || !contentRef.current) {
            alert("Escriu instrucció."); return;
        }
        const tempSpan = contentRef.current.querySelector<HTMLElement>(`span[data-ai-temp-id="${aiTemporaryMarkerId}"]`);
        if (tempSpan) {
            const instructionId = `ai-instr-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
            const originalText = aiSelectedText;
            tempSpan.removeAttribute('data-ai-temp-id');
            tempSpan.setAttribute('data-ai-instruction-id', instructionId);
            tempSpan.className = 'ai-prompt-target';
            const updatedHtml = contentRef.current.innerHTML;
            setConvertedHtml(updatedHtml);
            setAiInstructions(prev => [...prev, { id: instructionId, prompt: aiUserPrompt, originalText: originalText || undefined }]);
            console.log("Instrucció IA guardada:", { id: instructionId, prompt: aiUserPrompt });
            setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt('');
        } else {
            console.error("No marcador temporal IA."); alert("Error guardant instrucció.");
            setAiSelectedText(null); setAiTemporaryMarkerId(null); setAiUserPrompt('');
        }
    };
    ...
    const handleSaveConfiguration = async () => {
        ...
        // Neteja marcadors temporals IA abans de guardar
        let finalHtml = contentRef.current.innerHTML;
        const changed = unwrapTemporaryAiSpan(aiTemporaryMarkerId); // Usa la funció auxiliar
        if(changed){
            console.warn("S'ha eliminat un marcador temporal IA sense desar abans de guardar configuració.");
             finalHtml = contentRef.current.innerHTML; // Torna a llegir HTML netejat
             setConvertedHtml(finalHtml); // Actualitza vista
             setAiSelectedText(null); // Neteja estat temporal
             setAiTemporaryMarkerId(null);
        }

        const configuration = { /* ... (sense canvis) ... */
            baseDocxName: selectedFileName, excelInfo: { fileName: selectedExcelFileName, headers: excelHeaders, }, linkMappings: links, aiInstructions: aiInstructions, finalHtml: finalHtml
        };
                ...
                <aside className="w-80 flex-shrink-0 my-0 relative">
                    <div className="sticky top-4 p-4 bg-white rounded shadow-lg border max-h-[calc(100vh-2rem)] overflow-y-auto flex flex-col">
                        ...
                        <div className="flex justify-between items-center mb-3 pb-2 border-b flex-shrink-0">
                            ...
                            {/* Podríem afegir un botó de reset aquí si cal */}
                        </div>
                        ...
                        <div className="flex-grow overflow-y-auto space-y-4 pr-1 mb-4">
                            ...
                            {!convertedHtml && !isLoadingDocx && (
                                <div className="p-3 border border-dashed rounded">
                                    ...
                                    {/* ... (Input DOCX) ... */}
                                    <div className="flex flex-col items-start gap-2"> <label htmlFor="docxInputSidebar" className={`inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-xs sm:text-sm font-medium rounded shadow-sm text-white whitespace-nowrap bg-blue-600 hover:bg-blue-700 cursor-pointer`}> Selecciona DOCX </label> <input type="file" id="docxInputSidebar" onChange={handleFileChange} accept=".docx" className="hidden" /> {selectedFileName && (<span className="text-xs italic">({selectedFileName})</span>)} </div> {docxError && (<p className="text-xs text-red-600 mt-2">{docxError}</p>)}
                                </div>
                            )}
                            ...
                            {convertedHtml && excelHeaders.length === 0 && (
                                 <div className="p-3 border border-dashed rounded">
                                    <p className="text-sm font-medium mb-2">Pas 2: Carregar Excel (Opcional)</p>
                                    {/* ... (Input Excel) ... */}
                                     <div className="flex flex-col items-start gap-2"> <label htmlFor="excelInputSidebar" className={`... ${isParsingExcel ? 'bg-gray-400':'bg-green-600 hover:bg-green-700'} ...`}> {isParsingExcel ? 'Processant...' : (selectedExcelFileName ? 'Canvia Fitxer' : 'Selecciona Excel')} </label> <input type="file" id="excelInputSidebar" onChange={handleExcelFileChange} accept=".xlsx, .xls" className="hidden" disabled={isParsingExcel} /> {selectedExcelFileName && !isParsingExcel && (<span className="text-xs italic">({selectedExcelFileName})</span>)} </div> {isParsingExcel && (<div className="mt-2"><p className="text-green-600 animate-pulse text-xs">...</p></div>)} {excelError && (<p className="text-xs text-red-600 mt-2">{excelError}</p>)} {!selectedExcelFileName && !isParsingExcel && !excelError && (<p className="text-xs text-gray-400 mt-2 italic">...</p>)}
                                 </div>
                             )}
                                    ...
                                    <div className="pt-4 border-t border-indigo-200">
                                        ...
                                        {aiSelectedText && aiTemporaryMarkerId && (
                                            <div className='p-3 border rounded bg-indigo-50 space-y-3 border-indigo-200'>
                                                {/* ... (UI IA: text seleccionat, prompt, botó guardar) ... */}
                                                <div><label className='block text-xs ...'>Al text seleccionat:</label> <p className="text-xs bg-white p-2 border rounded max-h-20 overflow-y-auto" title={aiSelectedText}>{aiSelectedText.substring(0, 100)}{aiSelectedText.length > 100 ? '...' : ''}</p></div> <div> <label htmlFor="aiPrompt" className='block text-xs ...'>Escriu la instrucció:</label> <textarea id="aiPrompt" rows={4} value={aiUserPrompt} onChange={(e) => setAiUserPrompt(e.target.value)} placeholder="Ex: Resumeix, Fes més formal..." className="w-full p-2 border rounded text-xs" /> </div> <button onClick={handleSaveAiInstruction} disabled={!aiUserPrompt.trim()} className={`w-full ... ${!aiUserPrompt.trim() ? 'bg-gray-400 ...' : 'bg-indigo-600 hover:bg-indigo-700'}`} > Guardar Instrucció IA </button>
                                            </div>
                                        )}
                                        {/* Missatge si no hi ha text seleccionat per IA */}
                                        {!aiSelectedText && (
                                            <p className="text-xs text-gray-400 italic p-2 text-center">Selecciona text al document (sense capçalera Excel activa) per afegir una instrucció IA.</p>
                                        )}
                                        ...
                                        {aiInstructions.length > 0 && (
                                            <div className="mt-4 pt-4 border-t">
                                                ...
                                                <ul className="space-y-2 text-xs max-h-32 overflow-y-auto pr-1">
                                                    {aiInstructions.map(instr => ( <li key={instr.id} className="p-2 border rounded bg-indigo-50 text-gray-700"> <span className="block font-medium text-indigo-800">ID: {instr.id.substring(9,14)}...</span> <span className="block italic">"{instr.prompt}"</span> </li> ))}
                                                </ul>
                                            </div>
                                        )}
                                    </div>
