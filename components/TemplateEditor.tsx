  ...
  const handleSaveConfiguration = async () => {
    ...
    try {
      ...
      const response = await fetch(
        mode === 'edit' && initialTemplateData?.id
          ? `/api/update-template/${initialTemplateData.id}`
          : '/api/save-configuration',
        {
          method: mode === 'edit' && initialTemplateData?.id ? 'PUT' : 'POST',
          headers: {
  ...
  return (
    <main className="flex min-h-screen w-full flex-col items-center p-4 sm:p-8 bg-gray-100">
      ...
      <div id="print-header" className="hidden print:block w-full max-w-4xl mx-auto mb-4 text-center text-xs text-gray-500">
        Configuració Plantilla - {new Date().toLocaleDateString()}
      </div>
        ...
        <div className={`flex-grow print-content bg-white shadow-lg rounded-sm p-8 md:p-12 lg:p-16 my-0 ${!isMounted ? 'mx-auto max-w-3xl' : ''}`}>
            ...
            {isMounted && convertedHtml ? (
              ...
              !isLoadingDocx && !docxError && (
                <p className="text-gray-400 italic text-center py-10">
                  Comença seleccionant un fitxer DOCX al panell lateral.
                </p>
              )
            )}
          </div>
          ...
          {(links.length > 0 || aiInstructions.length > 0) && (
            <div className="mt-6 border-t border-gray-200 pt-4 space-y-4 text-xs max-w-5xl mx-auto">
              {links.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-purple-600 mb-1">Placeholders Excel:</h3>
                  <ul className="list-disc list-inside text-gray-600">
                    {links.map(l => (
                      <li key={l.id} className="mb-1">
                        <code>{l.selectedText}</code> <span className="text-gray-500">{'>'}</span>{' '}
                        <strong>{l.excelHeader}</strong>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="sticky top-4 p-4 bg-white rounded shadow-lg border max-h-[calc(100vh-2rem)] overflow-y-auto flex flex-col">
              ...
              <div className="flex-grow overflow-y-auto space-y-4 pr-1 mb-4">
                    ...
                    <div className="flex flex-col items-start gap-2">
                      <label
                        ...
                        className={`inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-xs sm:text-sm font-medium rounded shadow-sm text-white whitespace-nowrap ${
                          isLoadingDocx ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 cursor-pointer'
                        }`}
                      >
                      ...
                      />
                      {selectedFileName && !isLoadingDocx && (
                        <span className="text-xs text-gray-500 italic">({selectedFileName})</span>
                      )}
                    </div>
                    ...
                    <div className="p-3 border rounded border-gray-200">
                      ...
                      {selectedExcelHeader && (
                        <p className="text-xs bg-blue-50 p-2 rounded border border-blue-200">
                          <strong className="block mb-1">2.</strong> Selecciona text al document per{' '}
                          <strong className="text-red-600">REEMPLAÇAR</strong> per:{' '}
                          <span className="font-semibold italic">{selectedExcelHeader}</span>
                        </p>
                      )}
                      {!selectedExcelHeader && (
                        <p className="text-xs text-gray-500 p-2">
                          <strong className="block mb-1">INFO:</strong> Clica capçalera + selecciona text (vincular Excel). O clica un paràgraf (per instrucció IA).
                        </p>
                      )}
                      <div className="mt-2 text-center text-xs">
                        <label
                          htmlFor="excelInputSidebarChange"
                          className="text-blue-600 hover:text-blue-800 underline cursor-pointer"
                        >
                          Canviar Excel ({selectedExcelFileName || 'cap'})
                        </label>
                        <input
                          type="file"
                          id="excelInputSidebarChange"
                          onChange={handleExcelFileChange}
                          accept=".xlsx, .xls"
                          className="hidden"
                          disabled={isParsingExcel}
                ...
                {convertedHtml && (
                  <div className="p-3 border rounded border-gray-200">
                    ...
                    <button
                      type="button"
                      className={`mb-2 px-4 py-2 rounded font-semibold text-xs transition border ${
                        iaInstructionsMode
                          ? 'bg-indigo-600 text-white border-indigo-700'
                          : 'bg-white text-indigo-700 border-indigo-400'
                      } hover:bg-indigo-50`}
                      onClick={() => {
                        setIaInstructionsMode(prev => !prev);
                      }}
                    ...
                    {aiTargetParagraphId && (
                      <div className="mt-2 pt-3 border-t border-indigo-200 space-y-3">
                        <p className="text-xs font-medium text-indigo-700">
                          Editant paràgraf ID: <code className="text-xs">{aiTargetParagraphId.substring(0, 10)}...</code>
                        </p>
                        <div>
                          <label htmlFor="aiPrompt" className="block text-xs font-medium text-gray-600 mb-1">
                            Instrucció per la IA:
                          </label>
                          <textarea
                            id="aiPrompt"
                            rows={4}
                            ...
                            placeholder="Ex: Resumeix aquest paràgraf en una frase"
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            disabled={
                              !aiUserPrompt.trim() &&
                              !(aiInstructions.find(i => i.id === aiTargetParagraphId)?.prompt)
                            }
                            className={`flex-grow px-3 py-1.5 text-xs font-medium rounded shadow-sm text-white transition ${
                              !aiUserPrompt.trim() &&
                              !(aiInstructions.find(i => i.id === aiTargetParagraphId)?.prompt)
                          ...
                          <button
                            onClick={handleCancelAiInstruction}
                            className="px-3 py-1.5 text-xs text-gray-600 bg-gray-100 hover:bg-gray-200 border rounded"
                          >
                            Cancel·lar
                          </button>
                        </div>
                      ...
                      <div className="mt-4 pt-4 border-t">
                        ...
                        <ul className="space-y-2 text-xs max-h-32 overflow-y-auto pr-1">
                          {aiInstructions.map((instr, index) => (
                            <li
                              key={instr.id}
                              className={`p-2 border rounded bg-indigo-50 text-gray-700 cursor-pointer transition ${
                                iaInstructionsMode
                                  ? 'hover:border-dashed hover:border-2 hover:border-indigo-700'
                                  : 'hover:bg-indigo-100'
                              }`}
                              onClick={() => {
                                setAiTargetParagraphId(instr.id);
                            ...
                            >
                              ...
                              <span className="block text-gray-500 text-[10px]">
                                (Pàrr. ID: {instr.id.substring(2, 8)}...)
                              </span>
              ...
              <div className="mt-auto pt-4 border-t flex-shrink-0 space-y-2">
                <button
                  ...
                  disabled={saveStatus === 'saving' || !convertedHtml}
                  className={`w-full px-4 py-2 text-sm font-medium rounded shadow-sm transition ${
                    saveStatus === 'saving' || !convertedHtml
